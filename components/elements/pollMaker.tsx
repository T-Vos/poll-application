...
function PollMaker(props:{pollQuestions:Poll[], onAddQuestion:any, onQuestionChange:any}) {
    ...
    return (
        ...
        <div className={styles.pollQuestions}>
                <input onChange={handleChange} value={newPollQuestion} type="text" onKeyPress={x => handleKeyPress(x)} onBlur={x => addQuestionToList(x.target.value)}></input>
                <p>Enter questions by pressing enter or using tab</p>
                <hr />
                {props.pollQuestions.map(pollQuestion =>{
                    return(
                        <input onChange={x => props.onQuestionChange(x.target.value,pollQuestion.pollId)} key={`polling-${pollQuestion.pollId}`} type="text" value={pollQuestion.pollText} />
                    )
                })
                }
        </div>
